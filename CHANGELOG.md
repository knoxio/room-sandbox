...
### Fixed
- `kill_agent` now kills all matching PIDs instead of only the first
- `restart` polls for actual process death (up to 15s) instead of a blind 3-second sleep
- `compose up` auto-recovers from stale container/image references by force-removing and retrying
